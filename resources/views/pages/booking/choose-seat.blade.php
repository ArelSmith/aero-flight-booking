                        ...
                        <div class="flex flex-col rounded-[20px] border border-[#E8EFF7] p-5 gap-5">
                            ...
                            <div class="flex items-center rounded-[20px] gap-[14px]">
                                <div class="flex w-[120px] h-[100px] shrink-0 rounded-[20px] overflow-hidden">
                                    <img src="{{ asset('assets/images/thumbnails/' . $tier->class_type . '-seat.png') }}"
                                        class="w-full h-full object-cover" alt="icon">
                                </div>
                                <div>
                                    <p class="font-bold text-xl leading-[30px]">{{ Str::ucfirst($tier->class_type) }} Class</p>
                                    <p class="text-garuda-grey mt-1">{{ 'Rp. ' . number_format($tier->price, 2, ',', '.') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div id="Plane" class="relative flex w-[558px] shrink-0 mt-[30px] mx-auto">
                ...
                <div class="relative flex flex-col justify-end">
                    ...
                    <form action="passenger-details.html" class="relative px-[56px] pb-[60px]" id="form-seat">
                        ...
                        <div id="Seats-Options" class="flex flex-wrap w-full gap-y-8 {{ $tier->class_type == 'business' ? 'gap-x-10 px-[33px]' : 'gap-x-[14px]' }}">
                          @foreach ($flight->seats->where('class_type', $tier->class_type) as $seat)
                              <label 
                              class="group relative flex w-[55px] h-[52.25px] shrink-0 {{ $tier->class_type == 'business' ? "[&:nth-child(4n+2)]:mr-10" : "[&:nth-child(6n+3)]:mr-[46px]" }}" data-seat={{ $seat->name }} data-seat-id="{{ $seat->id }}">
                              <input type="checkbox" name="seat"
                                class="seat-checkbox absolute top-1/2 left-1/2 opacity-0" @if (!$seat->is_available) disabled @endif @if ($seat->is_available)  @endif >

                              <img src="{{ asset('assets/images/icons/seat.svg') }}" alt="seat" class="absolute w-full h-full object-contain opacity-100 group-has-[:checked]:opacity-0 group-has-[:disabled]:opacity-0 transition-all duration-300">
                              
                              <img src="{{ asset('assets/images/icons/seat-choosed.svg') }}" alt="seat" class="absolute w-full h-full object-contain opacity-0 group-has-[:checked]:opacity-100 group-has-[:disabled]:opacity-0 transition-all duration-300">
                              
                              <img src="{{ asset('assets/images/icons/seat-disabled.svg') }}" alt="seat" class="absolute w-full h-full object-contain opacity-0 group-has-[:disabled]:opacity-100 transition-all duration-300">

                              <p class="relative flex items-center justify-center h-full w-full pb-[8.25px] font-semibold text-[16.5px] leading-[24.75px] text-premiere-black group-has-[:checked]:text-white">{{ $seat->name }}</p>
                            </label>
                          @endforeach
                        </div>
                        <button type="submit"
