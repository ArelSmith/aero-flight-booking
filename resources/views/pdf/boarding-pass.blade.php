    ...
    <div class="boarding-pass">
        <div class="header-flight-info">
            <div class="from">
                ...
                <div>{{ $transaction->flight->segments->first()->time->format('d M Y') }}</div>
            </div>
            <div class="to">
                <div>TO:</div>
                <div>{{ $transaction->flight->segments->last()->name }}</div>
                <div>{{ $transaction->flight->segments->last()->time->format('d M Y') }}</div>
            </div>
        </div>

        <div class="details">
            <div>
                <div class="info-item">Passenger: <strong>{{ $transaction->name }}</strong></div>
                ...
                <div class="info-item">Class: <strong>{{ Str::ucfirst($transaction->class->class_type) }}</strong></div>
            </div>
            <div>
                <div class="info-item">Seats:</div>
                <ul>
                  @foreach ($transaction->passengers as $passenger)
                    <li>{{ $passenger->name }} - Seat: {{ $passenger->seat->name }}</li>
                  @endforeach
                </ul>
            </div>
        </div>
